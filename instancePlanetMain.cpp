...
int main() {
	...
	GLFWwindow* window = glfwCreateWindow(SCR_WIDTH, SCR_HEIGHT, "Explode", NULL, NULL);
	...
	Shader astShader("./Shaders/Instancing/asteroids.vs", "./Shaders/Instancing/asteroids.fs");
	...
	while (!glfwWindowShouldClose(window))
	{
		...
		float vis = glm::sin(currentFrame);
		...
		astShader.use();
		astShader.setMat4("projection", projection);
		astShader.setMat4("view", view);
		planetShader.use();
		...
		// draw meteorites
		astShader.use();
		astShader.setInt("texture_diffuse1", 0);
